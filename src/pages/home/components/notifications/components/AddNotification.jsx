...
const AddNotification = ({
    ...
    const saveData = async (values) => {
        try {
            ...
            if (response) {
                await saveAnnousmentToFirebase(response, values)
            }
        } catch (error) {
            console.log(error);
        }
    ...
    // save annousments to firebase
    const saveAnnousmentToFirebase = async (response, values) => {
        try {
            const docRef = doc(dbChatBot, "announcements", response._id);
            await setDoc(docRef, response);
            const newsData = {
                typeOfNotification: "announcements",
                announcementAr: values.announcementAr,
                announcementEn: values.announcementEn,
                titleAr: values.titleAr,
                titleEn: values.titleEn,
                date: values.date ? moment(values.date.toDate()) : moment(announcementDate),
                link: values.link,
                id: response._id
            }
            const callableResponse = httpsCallable(functions, "newsNotification")
            await callableResponse(newsData)
            message.success("Successfully add news")
        } catch (error) {
    ...
    async function saveNewsToFirebase(values) {
        ...
        try {
            ...
            const callableResponse = httpsCallable(functions, "sendAnnouncementNotification")
            const response = await callableResponse(newsData)
            message.success("Successfully add news")
        } catch (error) {
            console.error("Error adding document: ", error);
        }
